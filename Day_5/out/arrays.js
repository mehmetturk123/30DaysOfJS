...
const moreThan5Elements = [1, 2, 3, 4, 5, 6, 7, 8];
...
const mixedDataTypes = [15, 'Hello', true, { skill: "yes" }, { action: true }, 23];
...
const firstIndex = itCompanies.findIndex(str => findCharacter(str, 'o'));
const lastIndex = itCompanies.findLastIndex(str => findCharacter(str, 'o'));
const wantedArray = itCompanies.slice(firstIndex, lastIndex + 1);
console.log(wantedArray.toString());
function findCharacter(value, key) {
    const re = new RegExp(key, 'g');
    const regArray = value.match(re);
    return (regArray?.length == 2) ? true : false;
}
//Sort the array using sort() method
const numberArray = [0, 8, 15, 7, 16, 25, 36, 59, 98];
...
function middleIndexCalculator(length) {
    if ((length - 1) % 2 == 0) {
        return ((length - 1) / 2);
    }
    else {
...
const itCompaniesForShifting = ['Facebook', 'Google', 'Microsoft', 'Apple', 'IBM', 'Oracle', 'Amazon'];
itCompaniesForShifting.shift();
console.log(itCompaniesForShifting.toString());
//Remove the middle IT company or companies from the array
const tempItCompanies = ['Facebook', 'Google', 'Microsoft', 'Apple', 'IBM', 'Oracle', 'Amazon'];
const middleIndexForRemove = middleIndexCalculator(tempItCompanies.length);
if (typeof (middleIndexForRemove) === 'number') {
    ...
    tempItCompanies.pop();
    const middleIndex01 = middleIndexCalculator(tempItCompanies.length);
...
const tempItCompanies02 = ['Facebook', 'Google', 'Microsoft', 'Apple', 'IBM', 'Oracle', 'Amazon'];
