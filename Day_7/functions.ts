...
function bubbleSort(arr: any[], startIndex: number = 0): void {
  ...
  for (let index = startIndex; index < arr.length - 1; index++) {
    [arr[startIndex + counter], arr[startIndex + counter + 1]] = [
      arr[startIndex + counter + 1],
      arr[startIndex + counter],
    ];
    counter++;
  }
...
/** Function returns time in this format: 08/01/2020 04:08 */
...
function userIdGeneratedByUser(): void {
  const firstInput: string | null = window.prompt(
